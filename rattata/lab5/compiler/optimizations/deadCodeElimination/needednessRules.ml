...
let needednessRule3 (lineToPredecessorsArray : (int list) array) 
  ...
  match (indexedProg.(currLine)) with
    Tmp2AddrMov(s,arg,loc)->
      (match (arg,loc) with
         (TmpConst(_),_)->[]
        |(TmpLoc(TmpVar(Tmp t1)),TmpVar(Tmp t2))->
            if 
            L.exists (fun t -> t = t2) (lineToNeededTempsArray.(succLine))
            then [t1] else []
        |(TmpLoc(TmpDeref(Tmp t1)),TmpVar(Tmp t2))->
            if 
            L.exists (fun t -> t = t2) (lineToNeededTempsArray.(succLine))
            then [t1] else []
        |(TmpLoc(TmpVar(Tmp t1)),TmpDeref(Tmp t2))->
            if 
            L.exists (fun t -> t = t2) (lineToNeededTempsArray.(succLine))
            then [t1] else []
        |(TmpLoc(TmpDeref(Tmp t1)),TmpDeref(Tmp t2))->
            if 
            L.exists (fun t -> t = t2) (lineToNeededTempsArray.(succLine))
            then [t1] else [])
   |Tmp2AddrPtrBinop(op,arg,loc)->
      (match (arg,loc) with
         (TmpConst(_),TmpVar(Tmp t1)) -> 
            if 
            L.exists (fun t -> t = t1) (lineToNeededTempsArray.(succLine))
            then [t1] else []
        |(TmpConst(_),TmpDeref(Tmp t1)) -> 
            if 
            L.exists (fun t -> t = t1) (lineToNeededTempsArray.(succLine))
            then [t1] else [])
...
let rec getNeededTemps (indexedProg : tmp2AddrInstr array) 
  ...
  let neededFromRule2 = needednessRule2 linesToPredecessorsArray indexedProg linesToNeededTempsArray succLine currLine in
  let neededFromRule3 = needednessRule3 linesToPredecessorsArray indexedProg linesToNeededTempsArray succLine currLine in
  let () = linesToNeededTempsArray.(currLine) <- (neededFromRule1 @ neededFromRule2 @ neededFromRule3) in
