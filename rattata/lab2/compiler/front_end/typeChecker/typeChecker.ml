...
let rec tc_expression env (expression : A.untypedPostElabExpr) =
      ...
      (match op with
         GT -> 
          (match (tcExpr1, tcExpr2) with
           ...
           | _ -> ErrorMsg.error None 
                        ("greater than expression didn't typecheck \n");
                  raise ErrorMsg.Error)
       | LT -> 
          (match (tcExpr1, tcExpr2) with
       ...
       | DOUBLE_EQ -> 
          (match (tcExpr1, tcExpr2) with
           ...
           | _ -> ErrorMsg.error None 
                        ("double equals expressions didn't typecheck \n");
                  raise ErrorMsg.Error)
       | LOG_AND -> 
          (match (tcExpr1, tcExpr2) with
             (BoolExpr(exp1), BoolExpr(exp2)) -> BoolExpr(LogAnd(exp1, exp2))
           | _ -> ErrorMsg.error None 
                        ("logical and expressions didn't typecheck \n");
                  raise ErrorMsg.Error)
       | IntBinop(intOp) -> 
           (match (tcExpr1, tcExpr2) with
              (IntExpr(exp1), IntExpr(exp2)) -> IntExpr(ASTBinop(exp1, intOp, exp2))
            | _ -> ErrorMsg.error None 
                        ("int binop expressions didn't typecheck \n");
                   raise ErrorMsg.Error))
  ...
  | A.UntypedPostElabTernary(e1, e2, e3) ->
      ...
      (match (tcExpr1, tcExpr2, tcExpr3) with
             (BoolExpr(exp1), IntExpr(exp2), IntExpr(exp3)) -> 
               IntExpr(IntTernary(exp1, exp2, exp3))
           | (BoolExpr(exp1), BoolExpr(exp2), BoolExpr(exp3)) -> 
               BoolExpr(BoolTernary(exp1, exp2, exp3))
           | _ -> (ErrorMsg.error None 
                      ("ternary expression didn't typecheck \n");
                  raise ErrorMsg.Error))

let rec tc_statements env (untypedAST : untypedPostElabAST) (ret : bool) 
                          (typedAST : typedPostElabAST) =
  ...
  | A.UntypedPostElabDecl(id, typee)::stms ->
      (match M.find env id with
                   Some _ -> (ErrorMsg.error None 
                                  ("redeclared variable " ^ id ^ "\n");
                              raise ErrorMsg.Error)
                 | None -> (let newMap = M.add env id (typee, false) in 
                    tc_statements newMap stms ret 
                    ((TypedPostElabDecl(id, typee)) :: typedAST)))
                 ...
                 Some(typee, _) -> 
                   (match (tcExpr, typee) with
                          (IntExpr(_), INT) -> 
                            ...
                            tc_statements newMap stms ret 
                            ((TypedPostElabAssignStmt(id, tcExpr)) :: typedAST)
                        | (BoolExpr(_), BOOL) -> 
                            let newMap = M.add env id (typee, true) in              
                            tc_statements newMap stms ret 
                            ((TypedPostElabAssignStmt(id, tcExpr)) :: typedAST)
                        | _ -> (ErrorMsg.error None 
                                  ("assignment expression didn't typecheck \n");
                               raise ErrorMsg.Error))
               | None -> (if ((isValidVarDecl id))
                          then (let newMap = M.add env id (INT, true) in              
                            tc_statements newMap stms ret 
                            ((TypedPostElabAssignStmt(id, tcExpr)) :: typedAST))
                          else (ErrorMsg.error None 
                                    ("undeclared test variable " ^ id ^ "\n");
                                raise ErrorMsg.Error))))
               ...
               let newenv = M.mapi env 
                    ...
                    (fun ~data:value ->
                      (match (M.find env1 id, M.find env2 id) with
                       ...
                       | (_, _) -> value))) in
               tc_statements newenv stms newret 
               ((TypedPostElabIf(exp1, List.rev newast1, List.rev newast2)) 
                                :: typedAST)
  ...
  | A.UntypedPostElabWhile(e, ast1, untypedInitAst)::stms -> 
      ...
      (match tcExpr with
             BoolExpr(exp1) ->                
               ...
               ((TypedPostElabWhile(exp1, List.rev newast1)) 
                        :: (newast2 @ typedAST))
  ...
  | A.UntypedPostElabReturn(e)::stms -> 
      ...
      (match tcExpr with
             IntExpr(exp1) -> tc_statements newMap stms true 
                              ((TypedPostElabReturn(exp1)) :: typedAST)
